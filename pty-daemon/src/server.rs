...
impl Server {
    ...
    /// Handle WebSocket Detach — decrement ws_client_count only.
    ///
    /// WS detach does NOT touch the Unix session state (ETerm's attach is independent).
    fn handle_ws_detach(&mut self, _kq: RawFd, _client_id: u64, session_id: Uuid) -> Response {
        if let Some(session) = self.sessions.get_mut(&session_id) {
    ...
    /// Handle WebSocket client disconnect — decrement ws_client_count.
